...
urlGet = "http://127.0.0.1/prsi/Project_AquaPark/api/api.php?nome=vento&type=valor"
...
def onGetHTTPDone(status, data):
	...
	if status == 200:
		...
		if  wind: 
			...
			blower_state = 1
			lcd_state = "Vento: Detect"
		else: 
			# Caso não for detetado vento
			blower_state = 2
			lcd_state = "Vento: No Detect"
	...
	customWrite(pinFan, blower_state)
	customWrite(LCD, lcd_state)
	#
	save_actuator('blower', blower_state)
	save_actuator('lcd_wind', lcd_state)
